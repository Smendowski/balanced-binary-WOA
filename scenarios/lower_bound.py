...
def main() -> None:
    ...
    results_output_file = "data/results/raw/lower_bound_results.csv"
    codecarbon_output_file = "data/results/raw/lower_bound_codecarbon.csv"
        ...
        for data_dir in data_dirs:
            ...
            for vm_num in vm_nums:
                ...
                for context in ["CPU_USAGE_PERCENT", "MEMORY_USAGE_PERCENT"]:
                    ...
                    df_train = df_train.loc[:, [context]]
                    df_test = df_test.loc[:, [context]]
                    ...
                    for objective_fn in objective_fns:
                        ...
                        tracker.start()
                        tic = time.time()
                        best_result = objective_fn(
                            df_train,
                            y_train,
                            df_test,
                            y_test,
                            np.ones(df_train.shape[1]),
                        )
                        tac = time.time()
                        tracker.stop()

                        result = {
                            "fs_method": "LowerBound",
                            "fs_method_kwargs": {},
                            "n_selected_features": len(df_train.columns.tolist()),
                            "score": best_result,
                            "time": tac - tic,
